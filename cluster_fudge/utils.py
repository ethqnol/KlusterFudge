...
def hamming(X:np.array, centroids:np.array) -> int: #np.array enforces compiler check on the type you're passing in (must be an np.array)
...
@njit(parallel=True, fastmath=True)
def jaccard(X:np.array, centroids:np.array) -> int: #np.array enforces compiler check on the type you're passing in (must be an np.array)
    assert X.shape[1] == centroids.shape[1] # (taking the first index of shape, should be equal to the first index of centroids. Shape's index of [1] is the number of columns!)the first point must have the same shape (same number of arguments) as the first centroid
